...
_LIT8( KTestMsg, "LogMan Service: Connection established.\n" );

CLoggingServerServer* CLoggingServerServer::NewLC()
{
    CLoggingServerServer* self = new (ELeave) CLoggingServerServer(EPriorityStandard);
...
CLoggingServerServer::CLoggingServerServer(CActive::TPriority aActiveObjectPriority)
        : CServer2(aActiveObjectPriority), iIsSerialConnected(EFalse),
          iBytesSent(0), iIsClosing(EFalse)
{
...
CSession2* CLoggingServerServer::NewSessionL(const TVersion& aVersion,const RMessage2& /*aMessage*/) const
{
    ...
    TVersion v(KLogServMajorVersionNumber,KLogServMinorVersionNumber,KLogServBuildVersionNumber);
...
{
    PRINTF( "AddAsyncMessageL" );
    PRINTF( "Adding asynchronous message %S", &aBuffer );
    this->iMessageQueue->AppendL(aBuffer);
...
TInt CLoggingServerServer::ConnectSerial()
{
    ...
    connection_string.Format( KConnFormat,
                              &(this->iConnectionInfo.iPortName),
    ...
    TMessageBuffer8 tmp; tmp.Copy( KTestMsg );
    err = SendMessage( tmp );
    if( err != KErrNone) {
